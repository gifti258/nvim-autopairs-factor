...
local function setup(opt)
    ...
    local rules = {
        ...
        quote("'", "'", { "-rust", "-factor" }):with_pair(cond.not_before_regex("%w")),
        ...
        bracket("[", "]", "-factor"),
        bracket("{", "}", "-factor"),
        bracket("( ", "-- )", "factor"):with_pair(cond.after_text(" ")),
        bracket("( ", "-- )", "factor"):with_pair(cond.after_regex("$")),
        bracket("( ", "-- ) ", "factor"):with_pair(cond.not_after_text(" ")):with_pair(cond.not_after_regex("$")),
        bracket("(( ", " ))", "factor"):with_pair(cond.after_text(" ")),
        bracket("(( ", " ))", "factor"):with_pair(cond.after_regex("$")),
        bracket("(( ", " )) ", "factor"):with_pair(cond.after_text(" ")):with_pair(cond.after_regex("$")),
        bracket("[", " ] ", "factor"):with_pair(cond.not_after_text(" ")),
        bracket("[", " ]", "factor"):with_pair(cond.after_text(" ")),
        bracket("{", " }", "factor"),
        Rule(
            ">[%w%s]*$",
